...
!function (t, e, i) {
    ...
    var PI_2 = Math.PI / 2;
    var PI_4 = Math.PI / 4;
    var RMF_MERIDIAN_BIN = (1 << 27);

    wproj.DEG2RAD = Math.PI / 180;
    wproj.DEG2INT = RMF_MERIDIAN_BIN / 180;
    wproj.RAD2INT = RMF_MERIDIAN_BIN / Math.PI;
    wproj.RAD2DEG = 1 / wproj.DEG2RAD;
    wproj.INT2DEG = 1 / wproj.DEG2INT;
    wproj.INT2RAD = 1 / wproj.RAD2INT;


    wproj.sinh=function(arg) {
    ...
    wproj.wgs_to_point = function(wgs) {
        var wx = Math.round(wproj.DEG2INT * wgs.lng);
        var wy = Math.round(wproj.RAD2INT * (Math.log(Math.tan(PI_4 +
            (wproj.DEG2RAD * wgs.lat * 0.5)))));
        return {"x": wx, "y": wy};
    }



    wproj.point_to_wgs = function(wpt) {
        var lng = wproj.INT2DEG * wpt.x;
        var lat = wproj.RAD2DEG * (2 * Math.atan(Math.exp(wproj.INT2RAD *
            wpt.y)) - PI_2);
        return {"lat": lat, "lng": lng};
    }

    wproj.tilexy_to_latlng = function (x, y,z) {

        var n =  Math.pow(2, z);
        var lng  = (x+.5)/ n * 360.0 - 180.0;
        var lat_rad = Math.atan(wproj.sinh(Math.PI * (1 - 2 * (y+.5)/ n)));
        var lat = wproj.RAD2DEG*lat_rad;

        return {"lat": lat, "lng": lng};
    }

    var wgs_pts_to_wpts = function(wgs_pts) {
        var ret = [];
        for(var i = 0; i < wgs_pts.length; i++)
            ret.push(wproj.wgs_to_point(wgs_pts[i]));
        return ret;
    }

    var wpts_to_wgs_pts = function(wpts) {
        var ret = [];
        for(var i = 0; i < wpts.length; i++)
            ret.push(wproj.point_to_wgs(wpts[i]));
        return ret;
    }
    ...
    o.Mixin = {}, o.Mixin.Events = {addEventListener: function (t, e, i) {
        ...
        //console.log(o.Util.template(this._url, o.extend({s: this._getSubdomain(t), z: t.z, x: t.x, y: t.y}, this.options)));

        if(this._url.indexOf("mapmyindia") != -1){

            var _ret = wproj.tilexy_to_latlng( t.x,t.y, t.z);
            /*console.log("lat="+_ret.lat);
            console.log("lon="+_ret.lng);*/

            var ret = wproj.wgs_to_point(_ret);
            var zf=Math.pow(2,20-t.z);
            var ssf=o.Browser.retina? '1': '0';
         /*   console.log("x="+ret.x);
            console.log("y="+ret.y);
            console.log("z="+zf);
            console.log("ssf="+ssf);
            console.log(o.Util.template(this._url, o.extend({ z: zf, x: ret.x, y: ret.y,s: ssf}, this.options)));*/
            return o.Util.template(this._url, o.extend({ z: zf, x: ret.x, y: ret.y,s: ssf}, this.options))
        }else{
    ...
    }, o.Control.Attribution = o.Control.extend({options: {position: "bottomright", prefix: '<a href="http://leafletjs.com" title="A JS library for interactive maps"></a>'}, initialize: function (t) {
